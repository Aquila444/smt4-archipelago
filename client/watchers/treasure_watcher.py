...
async def watch(location_tracker: LocationTracker):
...
async def treasure_handler(previous_bytes: bytes, current_bytes: bytes, location_tracker: LocationTracker):
...
def get_location_from_flag(location_list: list[SmtLocation], flag: str) -> int | None:
    return next((location.archipelago_id for location in location_list if location.flag == flag), None)
